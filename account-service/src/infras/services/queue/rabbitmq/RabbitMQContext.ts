        ...
        return await this._channel.consume(queue, async msg => {
            if (msg) {
                ...
                if (headers[QueueHeaderKey.UserId] && headers[QueueHeaderKey.RoleId] && headers[QueueHeaderKey.AuthType])
                    userAuth = new UserAuthenticated(headers[QueueHeaderKey.UserId] as string, headers[QueueHeaderKey.RoleId] as string, headers[QueueHeaderKey.AuthType] as string);
