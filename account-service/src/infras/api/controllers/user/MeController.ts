...
export class MeController {
    ...
    async uploadMyAvatar(@UploadedFile('avatar', { required: true, options: { storage } }) file: Express.Multer.File, @CurrentUser() userAuth: UserAuthenticated, @HandleOptionRequest() handleOption: HandleOption): Promise<UploadMyAvatarCommandOutput> {
        const param = new UploadMyAvatarCommandInput();
        param.file = file;
        return await this._uploadMyAvatarCommandHandler.handle(userAuth.userId, param, handleOption);
    }
}
